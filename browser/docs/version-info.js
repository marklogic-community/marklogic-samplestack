...
var getGitRepoInfo = function() {
  ...
  var match = buildParams.build.docs.repoUrl ?
      GITURL_REGEX.exec(buildParams.build.docs.repoUrl) :
      GITURL_REGEX.exec(currentPackage.repository.url);
  var git = {
    owner: match[1],
    repo: match[2]
  };
...
exports.currentVersion = buildParams.build.docs.branch ?
    { tag: buildParams.build.docs.branch } :
    getTaggedVersion() || getBuild();// getSnapshotVersion();
