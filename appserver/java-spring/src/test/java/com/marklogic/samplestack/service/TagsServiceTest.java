...
public class TagsServiceTest extends MarkLogicIntegrationTest {
	...
	public void testTagSuggestion() {
		...
		String[] suggestions = tagsService.suggestTags(ClientRole.SAMPLESTACK_CONTRIBUTOR);
		assertTrue("Need suggestions to test", suggestions.length > 0);
		
		suggestions = tagsService.suggestTags(ClientRole.SAMPLESTACK_CONTRIBUTOR, "ab");
		assertTrue("Need suggestions to test", suggestions.length > 0);
	...
	private void assertAllContain(String[] suggestions, String pattern) {
		for (String suggestion : suggestions) {
			assertTrue("Suggestion contains candidate", suggestion.contains(pattern));
		}
