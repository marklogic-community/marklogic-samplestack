...
public class DatabaseTransformsIT extends MarkLogicIntegrationTest {
	...
	private static String TEST_URI = "/dbtest/transform-doc.json";
	...
	public void answerPatchTransform() {
		...
		contribManager.write(TEST_URI, new JacksonHandle(input), answerTransform);
		...
		assertEquals("transform added an answer", "this is the text of my answer", answers.get(0).get("text").asText());
		assertEquals("transform added a user", Utils.joeUser.getUserName(), answers.get(0).get("owner").get("userName").asText());
