...
curl --digest -u admin:admin -X POST -d@'src/main/resources/app-package.json' -Hcontent-type:application/json http://localhost:8002/manage/v2/packages?pkgname=sasquatch
